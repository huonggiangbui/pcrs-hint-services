...
export class migrate1698947177529 implements MigrationInterface {
    name = 'migrate1698947177529'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."problem_language_enum" AS ENUM('python')`);
        await queryRunner.query(`CREATE TABLE "problem" ("id" SERIAL NOT NULL, "pk" character varying NOT NULL, "language" "public"."problem_language_enum" NOT NULL DEFAULT 'python', "typeExperiment" boolean, "crossover" boolean, "name" character varying, "description" character varying, "solution" character varying, "starter_code" character varying, CONSTRAINT "UQ_946344322fd0ba7a36dbd702a12" UNIQUE ("pk", "language"), CONSTRAINT "PK_119b5ca6f3371465bf1f0f90219" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."logger_action_enum" AS ENUM('request', 'reveal', 'close')`);
        await queryRunner.query(`CREATE TABLE "logger" ("id" SERIAL NOT NULL, "timestamp" TIMESTAMP NOT NULL DEFAULT now(), "submission" character varying NOT NULL, "action" "public"."logger_action_enum" NOT NULL DEFAULT 'request', "hintId" integer, "studentId" integer, CONSTRAINT "PK_46cad7e44f77ea2fa7da01e7828" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "student" ("id" SERIAL NOT NULL, "uid" character varying NOT NULL, "conditionVisibility" "public"."student_conditionvisibility_enum" NOT NULL DEFAULT 'experiment', "conditionType" "public"."student_conditiontype_enum" NOT NULL DEFAULT 'text', CONSTRAINT "PK_3d8016e1cb58429474a3c041904" PRIMARY KEY ("id"))`);
